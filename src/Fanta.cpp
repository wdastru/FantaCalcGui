...
unsigned int Fanta::setModulo(std::string & str, size_t k) {
	...
	for (size_t i = 1; i < 4; i++) {
		this->modulo[k][i] = atoi(STR_MOD->msk(str, " :-", i).c_str()); // lettura del modulo di gioco
		xx += this->modulo[k][i];
	}
...
/*
 ...
 // nome
 tmpPlayer.Nome = msk(str, delim, ColNomeCognome);

 // goal e rigori
 if (msk(str, delim, ColRuolo) == "P") {
 tmpPlayer.Ruolo = 0;
 tmpPlayer.GoalSubiti = fabs(
 atof(msk(str, delim, ColGoalSubiti).c_str()));
 tmpPlayer.GoalFatti = 0;
 tmpPlayer.RigoreParato = fabs(
 atoi(msk(str, delim, ColRigore).c_str()) / 3);
 tmpPlayer.RigoreSbagliato = 0;
 } else if (msk(str, delim, ColRuolo) == "D") {
 tmpPlayer.Ruolo = 1;
 tmpPlayer.GoalFatti = atof(msk(str, delim, ColGoalFatti).c_str()) / 3;
 tmpPlayer.GoalSubiti = 0;
 tmpPlayer.RigoreSbagliato = fabs(
 atoi(msk(str, delim, ColRigore).c_str()) / 3);
 tmpPlayer.RigoreParato = 0;
 } else if (msk(str, delim, ColRuolo) == "C") {
 tmpPlayer.Ruolo = 2;
 tmpPlayer.GoalFatti = atof(msk(str, delim, ColGoalFatti).c_str()) / 3;
 tmpPlayer.GoalSubiti = 0;
 tmpPlayer.RigoreSbagliato = fabs(
 atoi(msk(str, delim, ColRigore).c_str()) / 3);
 tmpPlayer.RigoreParato = 0;
 } else if (msk(str, delim, ColRuolo) == "A") {
 tmpPlayer.Ruolo = 3;
 tmpPlayer.GoalFatti = atof(msk(str, delim, ColGoalFatti).c_str()) / 3;
 tmpPlayer.GoalSubiti = 0;
 tmpPlayer.RigoreSbagliato = fabs(
 ...
 // fantavoto
 if (msk(str, delim, ColFantaVotoGazzetta) == "-")
 tmpPlayer.FantaVotoGazzetta = 0;
 else
 tmpPlayer.FantaVotoGazzetta = atof(
 msk(str, delim, ColFantaVotoGazzetta).c_str());

 // voto gazzetta
 if (msk(str, delim, ColVotoGazzetta) == "-")
 tmpPlayer.VotoGazzetta = 0;
 else if (msk(str, delim, ColVotoGazzetta) == "S.V.")
 tmpPlayer.VotoGazzetta = -1;
 else
 tmpPlayer.VotoGazzetta = atof(msk(str, delim, ColVotoGazzetta).c_str());

 // goal decisivi ed altro
 tmpPlayer.GoalDecVitt = atoi(msk(str, delim, ColGoalDecVitt).c_str());
 if (tmpPlayer.GoalDecVitt != 0 && tmpPlayer.GoalFatti == 0)
 return PLAYER_GDV_NO_GOAL;

 tmpPlayer.GoalDecPar = atoi(msk(str, delim, ColGoalDecPar).c_str());
 if (tmpPlayer.GoalDecPar != 0 && tmpPlayer.GoalFatti == 0)
 return PLAYER_GDP_NO_GOAL;

 tmpPlayer.Assist = atoi(msk(str, delim, ColAssist).c_str());
 tmpPlayer.Autoreti = abs(atoi(msk(str, delim, ColAutoreti).c_str()) / 2);
 tmpPlayer.Amm = abs(int(atof(msk(str, delim, ColAmm).c_str()) / 0.5));
 tmpPlayer.Esp = abs(atoi(msk(str, delim, ColEsp).c_str()));
