...
void Fanta::initialize() {
	...
	for (size_t k = 0; k < 7; k++) {
		for (size_t i = 0; i < 4; i++) {
			inCampoModuli[k][i] = 0;
			distanzaModuli[k][i] = 0;
			disponibiliModuli[k][i] = 0;
		}
	}
					...
					if (cambioModulo) {
						...
						for (size_t j = 0; j < 7; j++) {
							...
							for (size_t i = 0; i < 4; i++) { // ruolo
								...
								for (size_t m = 0; m < Fanta::moduli[j][i];
										m++) {
